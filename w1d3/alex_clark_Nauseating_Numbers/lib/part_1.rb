...
def matrix_addition_reloaded(*matrix)
    return nil if matrix.any? {|arry| arry.length != matrix[0].length}
    new_arry = []
    matrix.transpose.each do |twod_arry|
        inner_arry = []
        twod_arry.transpose.each do |oned_arry|
            inner_arry << oned_arry.sum
        end
        new_arry << inner_arry
    end
    new_arry
...
def squarocol?(twod_arry)
    #row
    twod_arry.each do |arry|
        return true if arry.uniq.size <= 1
    end
    #col
    twod_arry.transpose.each do |arry|
        return true if arry.uniq.size <= 1
    end
    false
end
