def some?(arry, &prc)
    arry.each {|ele| return true if prc.call(ele)}
    false
end
...
def every?(arry, &prc)
    count = 0
    arry.each {|ele| count += 1 if prc.call(ele)}
    count == arry.length
end
...
def first_index(arry, &prc)
    arry.each {|ele| return arry.index(ele) if prc.call(ele)}
    nil
end
...
def xnor_select(arry, prc_1, prc_2)
...
def filter_out!(arry, &prc)
    arry.each_with_index do |ele, idx|
        arry.select! {|ele| !prc.call(ele)}
    end
end
...
def proctition(arry, &prc)
    arry.partition { |ele| prc.call(ele) }.flatten
end
...
def selected_map!(arry, prc_1, prc_2)
    arry.map! do |ele|
        if prc_1.call(ele)
            prc_2.call(ele)
        else
...
def proctition_platinum(arry, *procs)
    hash = {}
...
def picky_procipher(sent, hash)
    ...
    new_words = words.map do |word|
        new_word = word
        hash.each do |check, change|
            if check.call(word)
                new_word = change.call(new_word)
                break
            end
        end
        new_word
